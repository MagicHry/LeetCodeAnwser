...
object BigRootHeapSort {
     ...
     */
    fun organizeBigRoot(orignalArr: IntArray, arrLength: Int) {
        //找到最后一个ParentNode
    ...
    private fun organizeSubTree(currentParentIndex: Int, orignalArr: IntArray, arrLength: Int) {
        ...
        if (orignalArr[currentParentIndex] >= orignalArr[maxChildIndex]) {
            //parent大于等于最大孩子，则不动
            return
        }
        //如果小于，则要交换
        val parentValue = orignalArr[currentParentIndex]
        orignalArr[currentParentIndex] = orignalArr[maxChildIndex]
        orignalArr[maxChildIndex] = parentValue
        //由于我们修改了maxChildIndex处的值，maxChildIndex可能也是一个parentNode，要继续进行调整
        //直到我们撞见了叶子节点或者根本不用调整
        organizeSubTree(maxChildIndex, orignalArr, arrLength)
    }
